...
exports.login = async (req, res, next) => {
  try {
    ...
    // Check static admin credentials first (works even without database)
    if (
    ...
    // Try to query database for admin
    try {
      ...
      // Database is unavailable, return error
      console.warn('⚠️  Database unavailable for login, only static admin can login');
      return res.status(503).json({
        success: false,
        message: 'Database unavailable. Only static admin credentials accepted. Please check server configuration.',
      });
    }
  } catch (error) {
    next(error);
